...
class YOLOv7AuxHead(nn.Cell):
    ...
    def construct(self, x):
        ...
        for i in range(self.nl):
            ...
            out1 = mint.permute(out1.view(bs, self.na, self.no, ny, nx), (0, 1, 3, 4, 2))
            outs_1 += (out1,)

            out2 = self.m2[i](x[i + self.nl])
            out2 = mint.permute(out2.view(bs, self.na, self.no, ny, nx), (0, 1, 3, 4, 2))
            outs_2 += (out2,)
            ...
            if not self.training:  # inference
                ...
                z += (y.view(bs, -1, self.no),)
        outs = outs_1 + outs_2
        return outs if self.training else (mint.concat(z, 1), outs_1)
