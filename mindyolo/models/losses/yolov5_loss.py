...
class YOLOv5Loss(nn.Cell):
    ...
    def scatter_index_tensor(self, x, index):
        x_tmp = mint.permute(x.reshape((-1, x.shape[-1])), (1, 0))
    ...
    def construct(self, p, targets, imgs):  # predictions, targets
        ...
        for layer_index, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # Regression
                pxy = mint.sigmoid(pxy) * 2 - 0.5
                pwh = (mint.sigmoid(pwh) * 2) ** 2 * anchors[layer_index]
        ...
        loss = lbox + lobj + lcls
        loss_item = ops.stop_gradient(mint.stack((loss, lbox, lobj, lcls)))
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            j = mint.maximum(r, 1 / r).max(2) < self.hyp_anchor_t  # compare
            ...
            jk = mint.logical_and((gxy % 1 < g), (gxy > 1))  # .astype(ms.int32)
            lm = mint.logical_and((gxi % 1 < g), (gxi > 1))  # .astype(ms.int32)
            j, k = jk[:, 0], jk[:, 1]
            l, m = lm[:, 0], lm[:, 1]
            ...
            # Faster
            tag1, tag2 = mint.nn.Identity()(j), mint.nn.Identity()(k)
            tag1, tag2 = mint.tile(tag1[:, None], (1, 2)), mint.tile(tag2[:, None], (1, 2))
            ...
            tbox += (mint.concat((gxy - gij, gwh), 1),)  # box
