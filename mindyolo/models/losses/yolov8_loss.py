...
class YOLOv8Loss(nn.Cell):
    def __init__(self, box, cls, dfl, stride, nc, reg_max=16, tal_topk=10, **kwargs):
        ...
        self.bce = mint.nn.BCEWithLogitsLoss(reduction="none")
        self.hyp_box = box
        self.hyp_cls = cls
        self.hyp_dfl = dfl
    ...
    def construct(self, feats, targets, imgs):
        ...
        return loss.sum() * batch_size, ops.stop_gradient(
            mint.concat((loss.sum(keepdims=True), loss))
        )  # loss(box, cls, dfl)
    ...
    def bbox_decode(self, anchor_points, pred_dist):
        if self.use_dfl:
            ...
            pred_dist = mint.nn.functional.softmax(pred_dist, 3)  # ms version <= 1.8.1
            # (batch, anchors, 4, reg_max) @ (reg_max,) -> (batch, anchors, 4)
            _dtype = pred_dist.dtype
            pred_dist = mint.matmul(pred_dist.astype(ms.float16), self.proj.astype(ms.float16)).astype(_dtype)
...
class YOLOv8SegLoss(YOLOv8Loss):
    ...
    def construct(self, preds, target_box, target_seg):
        ...
        for i in range(batch_size):
            _fg_mask, _fg_mask_index = mint.topk(fg_mask[i].astype(ms.float16), self.max_object_num)
            _mask = target_seg[i]  # (mask_h, mask_w) if overlap else (n_gt, mask_h, mask_w)
            _mask_idx = target_gt_idx[i]  # (b, N) -> (N,)
            _mask_idx = mint.gather(_mask_idx, _fg_mask_index, axis=0)  # (max_object_num,)
    ...
    def single_mask_loss(self, gt_mask, pred, proto, xyxy, area, _fg_mask, _fg_mask_index):
        ...
        pred = mint.gather(pred, _fg_mask_index, axis=0)
        xyxy = mint.gather(xyxy, _fg_mask_index, axis=0)
        area = mint.gather(area, _fg_mask_index, axis=0)

        _dtype = pred.dtype
        pred_mask = mint.matmul(
        ...
        num_seg = mint.clamp(_fg_mask.sum(), min=1.0)
    ...
    def crop_mask(masks, boxes):
        ...
        """
        n, h, w = masks.shape
        x1, y1, x2, y2 = mint.chunk(boxes[:, :, None], 4, 1)  # x1 shape(n,1,1)
        r = mint.arange(w, dtype=x1.dtype)[None, None, :]  # rows shape(1,1,w)
        c = mint.arange(h, dtype=x1.dtype)[None, :, None]  # cols shape(1,h,1)

        return masks * mint.logical_and(
            mint.logical_and((r >= x1), (r < x2)),
            mint.logical_and((c >= y1), (c < y2))
        ).astype(x1.dtype)
