...
class YOLOv7Loss(nn.Cell):
    ...
    def construct(self, p, targets, imgs):
        ...
        for i in range(self.nl):  # layer index
            ...
            tobj = mint.zeros_like(pi[..., 0])  # target obj
            ...
            if self.nc > 1:  # cls loss (only if multiple classes)
                ...
                lcls += self.BCEcls(ps[:, 5:], t, mint.tile(tmask[:, None], (1, t.shape[1])))  # BCE
        ...
        loss = lbox + lobj + lcls
        return loss * bs, ops.stop_gradient(mint.stack((loss, lbox, lobj, lcls)))
    ...
    def build_targets(self, p, targets, imgs):
        ...
        cls_preds_ = mint.tile(
            mint.unsqueeze(cls_preds_, 1), (1, n_gt_max, 1, 1)
        ...
        matched_inds = mint.stack((matched_bs_inds.view(-1), matched_gt_inds.view(-1)), 1)  # (bs*nl*5*na*gt_max, 2)
    ...
    def find_3_positive(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            j = mint.maximum(r, 1.0 / r).max(2) < self.hyp_anchor_t  # compare # (na, nt)

            mask_m_t = mint.logical_and(j, mask_t[None, :]).view(-1)
            t = t.view(-1, 7)  # (na*nt, 7)

            # Offsets
            gxy = t[:, 2:4]  # grid xy
            gxi = gain[[2, 3]] - gxy  # inverse
            jk = mint.logical_and((gxy % 1.0 < g), (gxy > 1.0))
            lm = mint.logical_and((gxi % 1.0 < g), (gxi > 1.0))
            j, k = jk[:, 0], jk[:, 1]
            l, m = lm[:, 0], lm[:, 1]
            ...
            # Faster
            tag1, tag2 = mint.tile(j[:, None], (1, 2)), mint.tile(k[:, None], (1, 2))
            j_l = mint.logical_or(j, l).astype(ms.int32)
...
class YOLOv7AuxLoss(nn.Cell):
    ...
    def construct(self, p, targets, imgs):
        ...
        for i in range(self.nl):  # layer index
            ...
            tobj = mint.zeros_like(pi[..., 0])  # target obj
            tobj_aux = mint.zeros_like(pi_aux[..., 0])  # target obj
            ...
            if self.nc > 1:  # cls loss (only if multiple classes)
                ...
                lcls += 0.25 * self.BCEcls(
                    ps_aux[:, 5:], t_aux, mint.tile(tmask_aux[:, None], (1, t_aux.shape[1]))
                )  # BCE
    ...
    def find_5_positive(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            # t = t[j]  # filter
            mask_m_t = mint.logical_and(j, mask_t[None, :]).view(-1)  # filter
            t = t.view(-1, 7)  # (na*nt, 7)

            # Offsets
            gxy = t[:, 2:4]  # grid xy
            gxi = gain[[2, 3]] - gxy  # inverse
            jk = mint.logical_and((gxy % 1.0 < g), (gxy > 1.0)).astype(ms.int32)
            lm = mint.logical_and((gxi % 1.0 < g), (gxi > 1.0)).astype(ms.int32)
            j, k = jk[:, 0], jk[:, 1]
            l, m = lm[:, 0], lm[:, 1]
